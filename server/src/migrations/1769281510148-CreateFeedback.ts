...
export class CreateFeedback1769281510148 implements MigrationInterface {
    name = 'CreateFeedback1769281510148'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`feedback\` (\`id\` int NOT NULL AUTO_INCREMENT, \`isPositive\` tinyint NOT NULL, \`comment\` text NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`messageId\` int NULL, UNIQUE INDEX \`REL_326c785b6f3c1c5c7eb8c2ed13\` (\`messageId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`message\` ADD CONSTRAINT \`FK_58be890b9a1199e6da5037a994f\` FOREIGN KEY (\`sessionId\`) REFERENCES \`session\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`DROP INDEX \`REL_326c785b6f3c1c5c7eb8c2ed13\` ON \`feedback\``);
