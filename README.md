...
7. Navigate to server directory with command `cd server` 
8. Run `npm install` command which will install neccessary dependencies
9. Run `npm start` command which will run migrations and then start server
10. Navigate to client directory with command `cd ..` and `cd client` after 
11. Run `npm start` command which will install dependencies, then run linter, build application and then run it at localhost:8080
12. Wait until command is done
